...
(function (RT) {
  ...
  function rebuildFromRanges() {
    ...
    for (var i = 0; i < toks.length; i++) {
      ...
      var isBr = (t.t === 'br'), isSp = (t.t === 'sp');
      if (isBr || isSp) { out.push(t); acc += t.s.length; continue; }
      // 逐字符判是否被任一区间覆盖；同时判是否被强制区间覆盖（黄底标记）
      var len = te - ts, covered = [], anyC = false;
      for (var c = 0; c < len; c++) {
        var cp = ts + c, isCov = false;
        for (var k = 0; k < ranges.length; k++) {
          if (cp >= ranges[k][0] && cp < ranges[k][1]) { isCov = true; break; }
        }
        covered[c] = isCov; if (isCov) anyC = true;
      }
      if (!anyC) {
        out.push(t);
      } else {
        ...
        for (var c2 = 1; c2 <= len; c2++) {
          if (c2 === len || covered[c2] !== covered[runStart]) {
            out.push(mkTok(t, ts + runStart, c2 - runStart, acc + runStart, covered[runStart],
              isForcedRun(franges, ts + runStart, ts + c2, covered[runStart])));
            runStart = c2;
          }
        }
      }
  ...
  function mkTok(src, p, len, accP, isBlank, forced) {
    return {
      s: src.s.substring(p - src.p, (p - src.p) + len),
      t: src.t, p: accP, ck: src.ck, st: src.st,
      suggest: false, _blank: !!isBlank, forced: !!forced
    };
  }
  ...
  function bind() {
    document.body.addEventListener('click', function (e) {
      ...
      if (t.id === 'selAdd' || closestEl(t, '#selAdd')) {
        if (lastRange && lastRange.sOff != null) {
          ...
          clearSelPop(); window.getSelection().removeAllRanges();
          renderPractice();
        }
        return;
      }
    ...
    // 问题4：屏蔽设计模式下 #paper 内的原生右键菜单（避免"选中文字→右键搜索"
    // 抢占选区与挖空浮层），让用户拖选后只走挖空流程。
    document.addEventListener('contextmenu', function (e) {
      if (state.view === 'practice' && state.mode === 'design' && e.target && closestEl(e.target, '#paper')) {
        e.preventDefault();
      }
    });
    // 浮层自身不可被选中，且按下时阻止默认（防原生选区/搜索浮窗抢焦点）
    if (selPop) {
      selPop.addEventListener('mousedown', function (e) { e.preventDefault(); });
      selPop.setAttribute('style', (selPop.getAttribute('style') || '') + ';user-select:none;-webkit-user-select:none');
    }
